import { Denops, batch, option, autocmd, expandGlob, unknownutil } from './deps.ts';
...
class SnippetEngine {
    ...
    private async setMappingConfig(denops: Denops, config: Config): Promise<void> {
        await batch(denops, async (denops) => {
            for (const key of config.expand) {
                await denops.cmd(`imap ${key} <C-c>:call denops#request('olppet', 'expand', ['${key}'])<CR>`);
            }
            for (const key of config.jump_forward) {
                await denops.cmd(`imap ${key} <C-c>:call denops#request('olppet', 'jumpForward', ['${key}'])<CR>`);
            }
            for (const key of config.jump_backward) {
                await denops.cmd(`imap ${key} <C-c>:call denops#request('olppet', 'jumpBackward', ['${key}'])<CR>`);
            }
        });
    }
    ...
    public async expand(denops: Denops, key: string): Promise<void> {
    ...
    public async jumpForward(denops: Denops, key: string): Promise<void> {
        console.log('jumpForward');
    }

    public async jumpBackward(denops: Denops, key: string): Promise<void> {
